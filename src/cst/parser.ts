...
function makeParser(sourceKind: SourceKind): (line: string) => PyNode | boolean {
    ...
    let column = 0;
    let prefix = "";
    const tokenizer = new Tokenizer(false/*sourceKind === SourceKind.Single*/, function tokenizerCallback(type: Tokens, value: string, start: [number, number], end: [number, number], line: string): boolean | undefined {
